...
def get_album_image_path(self, filename):
  ...
  return '/'.join(['album', self.artist.stage_name, filename])


class Album(models.Model):
  #artist = models.ManyToManyField('Artist') #ForeignKey -> ManyToManyField & through: 여러 아티스트가 모일 경우 고려
...
class Song(models.Model): # Music -> Song : Music은 Album을 포함할 수 있는 의미도 있어서
  ...
  artist = models.CharField(max_length=150) # artist -> featuring : 피처링만 빼는게 좋음!
